...
<body>
						...
						<div class="col-md-12 grid-margin stretch-card">
							<div class="card">
								<div class="card-body">
									...
									<a
										href="/trading?stockCode=${stockCode}&stockName=${stockName}">
										<button type="button" class="btn btn-primary me-2"
											style="width: 100%; margin-top: 20px">매매</button>
									</a>
								</div>
							</div>
						</div>
								...
								<div class="card-body">
									...
									<div class="table-responsive">
										<table id="example" class="table">
											...
											<tbody>
												<c:forEach var="statement"
													items="${financialstatements.list}">
													<tr>
														<td>${statement.sj_nm}</td>
														<td>${statement.account_nm}</td>
														<td>${statement.thstrm_amount}</td>
														<td>${statement.frmtrm_amount}</td>
														<td>${statement.bfefrmtrm_amount}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
	...
	makeCharts(splitedData[here], "#box")// 페이지 로딩시 차트그림
	//makeCharts(splitedData2[now2], "#box2")

	window.addEventListener("resize", function() {
		reSize(splitedData, '#box', here)
	});
	/*window.addEventListener("resize", function() {
	...
	*/d3.select("#box").select(".right").on("click", function() {
		here = rightClick(splitedData, "#box", here)
	});
	d3.select("#box").select(".left").on("click", function() {
		here = leftClick(splitedData, "#box", here)
	}); //다른 날로 이동한 차트 그림/*
	...
	function rightClick(data, box, here) {
		here--;
		let remove = d3.select(box).selectAll(".del")
		console.log("@@@@")
		console.log(data)
		if (here > -1) {
			...
			console.log(box)
			makeCharts(data[here], box)
		} else {
			here = 0;
		}
		return here;
	}
	function leftClick(data, box, here) {
		let remove = d3.select(box).selectAll(".del")
		here++;
		console.log(data)
		console.log("@@@@")
